...
fn layout_showcase(root_node: Query<Entity, With<ShowcaseContainer>>, mut commands: Commands) {
    let row_color = Color::rgb(0.6, 0.7, 0.6);
    let column_color = Color::rgb(0.7, 0.6, 0.7);
    let root_entity = root_node.single();
    ...
    commands
        ...
        .row(|row| {
            row.sized_zone(
                ...
                |sized_zone| {
                    sized_zone.label(LabelConfig {
                        label: format!("{:?}", sized_zone.id()),
                        ..default()
                    });
            ...
            row.docking_zone(
                ...
                false,
                |tab_container| {
                    tab_container.add_tab("Tab 1".into(), |panel| {
            ...
            row.floating_panel(
                FloatingPanelConfig {
                    title: Some(String::from("Obnoxious Floating Panel")),
                    ..default()
                },
                FloatingPanelLayout {
