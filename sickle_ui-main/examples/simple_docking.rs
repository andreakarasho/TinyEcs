...
fn main() {
    App::new()
        ...
        .add_systems(PreUpdate, (exit_app_on_menu_item, print_hierarchy_on_menu_item, spawn_floating_zone))
        .add_systems(
            PreUpdate,
...
fn spawn_floating_zone(q_menu_items: Query<&MenuItem, (With<SpawnZoneButton>, Changed<MenuItem>)>,
root_node: Query<Entity, With<ShowcaseContainer>>,
mut commands: Commands,
) {
    let Ok(item) = q_menu_items.get_single() else {
    ...
    if item.interacted() {
        commands.ui_builder(root_entity).floating_panel(
                FloatingPanelConfig {
                    title: Some(String::from("Floating Panel")),
                    ..default()
                },
                FloatingPanelLayout {
                    size: Vec2 { x: 300., y: 300. },
                    position: Vec2 { x: 600., y: 200. }.into(),
                    droppable: true,
                    ..default()
                },
                |panel| {
