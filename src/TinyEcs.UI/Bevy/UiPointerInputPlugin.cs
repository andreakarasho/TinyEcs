...
public struct UiPointerInputPlugin : IPlugin
{
		...
		Query<Data<Scrollable, ComputedLayout>> scrollableQuery)
	{
		...
		for (int i = uiStack.Value.Count - 1; i >= 0; i--)
		{
			...
			// ComputedLayout positions already have scroll offsets applied (subtracted).
			// These are the final screen positions used for rendering, so we use them directly for hit testing.
